...
public class Setrep extends AbstractCommand {
	...
	public Setrep(ReputationDAO reputationDAO) {
		super("setrep",
				"Sets the number of rep of the specified user to the specified number in the specified database.");
	...
	public boolean onCommand(MessageReceivedEvent event, String s, String rawArguments, List<String> list) {
		if (event.getGuild().getIdLong() == Main.guildID) {
			...
			case 1:
				...
				event.getChannel().sendMessage("Set <@" + name.getId() + ">'s weekly rep balance to " + list.get(2))
						.queue();
				break;
			case 2:
				reputationDAO.monthlyCollection.updateOne(
						...
						Updates.set("repAmount", Integer.parseInt(list.get(2))));
				event.getChannel().sendTyping().queue();
				event.getChannel().sendMessage("Set <@" + name.getId() + ">'s monthly rep balance to " + list.get(2))
						.queue();
				break;
			case 3:
