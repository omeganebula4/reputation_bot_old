		...
		if (event.getGuild().getIdLong() == Main.guildID) {
			...
			if (list.size() == 0) {
				...
				SelectionMenu menu = SelectionMenu.create("leaderboard-selection")
						.setPlaceholder("Choose the leaderboard.") // shows the placeholder indicating what this menu is
																	// for
						.setRequiredRange(1, 1) // only one can be selected
						.addOption("Weekly Leaderboard", "weekly").addOption("Monthly Leaderboard", "monthly")
						.addOption("All-time Leaderboard", "alltime").build();
				event.getChannel().sendMessage("Select the leaderboard to be displayed.").setActionRow(menu).queue();
			}
