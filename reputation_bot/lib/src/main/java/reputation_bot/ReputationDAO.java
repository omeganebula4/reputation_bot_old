...
public class ReputationDAO {
		...
		public void run() {
			...
			try {
				while (cursor.hasNext()) {
					weeklyCollection.updateOne(Filters.and(Filters.eq("memberID", cursor.next().getMemberID()),
							Filters.eq("guildID", Main.guildID)), Updates.set("repAmount", 0));
				}
			} finally {
				cursor.close();
			}
	...
	MonthlyTimer timer = new MonthlyTimer(1, () -> {
		...
		try {
			while (cursor.hasNext()) {
				monthlyCollection.updateOne(Filters.and(Filters.eq("memberID", cursor.next().getMemberID()),
						Filters.eq("guildID", Main.guildID)), Updates.set("repAmount", 0));
			}
		} finally {
			cursor.close();
