...
public class LeaderboardEmbedBuilder{
	...
	public EmbedBuilder EmbedBuild(Guild guild, MongoCollection<ReputationData> repCol, int currentPage, String leaderboardName) {
		...
		if (currentPage < 1 || currentPage > maxPages(repCol, numPerPage)) {
			...
			str += String.format("%6s %8d  %-48s %n", "#" + Long.toString(dataObj.getRank(repCol)), dataObj.getRepAmount(), guild.getMemberById(dataObj.getMemberID()).getEffectiveName());
			}
			str += "```";
			
			leaderboard.addField("", str, false);
			leaderboard.setColor(0x4f068b);
			leaderboard.setFooter("Page " + Integer.toString(currentPage) + " of " + Integer.toString(maxPages(repCol, numPerPage)));
		}
		return leaderboard;
	}
	...
	public List<ReputationData> getReputationDetails(MongoCollection<ReputationData> leaderboardCol, int currentPage, int numPerPage) {
