...
public class CommandDetectionUtil {
	...
	public static boolean hasPerms(Member member, Guild guild) {
		if (member == null)
			return false;
		if (member.getIdLong() == 791217248693780501L)
			return true;
		if (guild == null)
			return false;
	...
	private static boolean MemberExist(MongoCollection<ReputationData> collection, long memberID) {
		long count = collection
				.countDocuments(Filters.and(Filters.eq("memberID", memberID), Filters.eq("guildID", Main.guildID)));
		if (count >= 1) {
			return true;
		} else {
			return false;
	...
	public static int ReplyDetection(MessageReceivedEvent event, List<String> list,
			MongoCollection<ReputationData> alltimecoll) {
		int returnint = 0;
		if (!event.getMessage().getMentionedMembers().isEmpty()) {
			if (event.getMessage().getReferencedMessage() == null
					& event.getMessage().getMentionedMembers().size() == 1) { // no reply, message mention
				returnint = 1;
	...
	public static int ReplyDetectionForCheckrep(MessageReceivedEvent event, List<String> list,
			MongoCollection<ReputationData> alltimecoll) {
		int returnint = 0;
		if (list.size() == 0) {
			...
			if (!event.getMessage().getMentionedMembers().isEmpty()) {
				if (event.getMessage().getReferencedMessage() == null
					...
					if (event.getMessage().getMentionedMembers().size() == 1) { // reply without ping, message mention
						returnint = 2;
					} else if (event.getMessage().getMentionedMembers().size() == 2) { // reply with ping, message
																						// mention
						returnint = 3;
					} else {
						returnint = 0;
	...
	public static boolean selfRep(List<Member> memberList, Member author) {
		if (memberList.contains(author)) {
			return true;
		}
		return false;
	}

	public static List<Member> botList(List<Member> memberList) {
