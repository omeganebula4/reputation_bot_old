...
class MemberConsumer implements Consumer<Member> {
	...
	public void accept(Member member) {
		if (!member.getUser().isBot()) {
			long count = reputationDAO.alltimeCollection.countDocuments(
					Filters.and(Filters.eq("memberID", member.getIdLong()), Filters.eq("guildID", Main.guildID)));
...
public class MemberHandler extends ListenerAdapter {
	...
	public void onGuildMemberJoin(GuildMemberJoinEvent event) {
		...
		if (!newMember.getUser().isBot()) {
			long count = reputationDAO.alltimeCollection.countDocuments(
					Filters.and(Filters.eq("memberID", newMember.getIdLong()), Filters.eq("guildID", Main.guildID)));
	...
	public void onGuildMemberRemove(GuildMemberRemoveEvent event) {
		...
		if (!leftMember.getUser().isBot()) {
			long count = reputationDAO.alltimeCollection
					.countDocuments(Filters.and(Filters.eq("memberID", leftMember.getIdLong()),
							Filters.eq("guildID", Main.guildID), Filters.eq("repAmount", 0)));

			if (count > 0) {
				reputationDAO.alltimeCollection.deleteMany(Filters.and(Filters.eq("memberID", leftMember.getIdLong()),
						Filters.eq("guildID", Main.guildID), Filters.eq("repAmount", 0)));
				reputationDAO.monthlyCollection.deleteMany(Filters.and(Filters.eq("memberID", leftMember.getIdLong()),
						Filters.eq("guildID", Main.guildID), Filters.eq("repAmount", 0)));
				reputationDAO.weeklyCollection.deleteMany(Filters.and(Filters.eq("memberID", leftMember.getIdLong()),
						Filters.eq("guildID", Main.guildID), Filters.eq("repAmount", 0)));
			}
		}
	}
